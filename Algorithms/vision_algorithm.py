...
class VisionAlgorithm:
    ...
    def __init__(self):
        ...
        self.points = np.array([[-5.5, 5, 0], [5.5, 5, 0], [-5.5, -5, 0], [5.5, -5, 0]]).reshape(-1, 3)/100 # Original points in meters.
    ...
    def minimize(self, pixel_pose, R, T, omega, vel):
        ...
        pixel_pose = np.reshape(pixel_pose.copy(), (-1, 2))
        print(pixel_pose)
        ...
        result = least_squares(self._reprojection_func, init_params, args=(pixel_pose, R, T))
        # print(result)
        return result
