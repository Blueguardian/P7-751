...
class EKF():
    ...
    def ekf(self, sensed_state, sensor_measurements, dt):
        ...
        if self.updateNumber == 0:  # First update.
        ...
        # Reinitialize State
        if self.updateNumber == 1:  # Second Update
        ...
        if self.updateNumber > 1:
            ...
            mx = [.0698 * cos(r) * sin(w) - .0698 * cos(w) * sin(r) * sin(p),
                  .0698 * cos(r) * cos(w) + .0698 * sin(r) * sin(p) * sin(w), -.0698 * cos(p) * sin(r)]
            my = [.0698 * cos(r) * cos(p) * cos(w) - .998 * cos(w) * sin(p),
                  .998 * sin(p) * sin(w) - .0698 * cos(r) * cos(p) * sin(w), -.998 * cos(p) - .0698 * cos(r) * sin(p)]
            mz = [-.0698 * cos(w) * sin(r) - .988 * cos(p) * sin(w) - .0698 * cos(r) * sin(p) * sin(w),
