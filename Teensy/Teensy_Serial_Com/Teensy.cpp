...
void Teensy::get_sensors_ranges() {
...
void Teensy::set_sensors_ranges(int ACCEL_FS, int GYRO_FS) {
...
void Teensy::serial_com_setup(){
    Serial.begin(115200);
    Serial.setTimeout(0);
}

void Teensy::await_for_connection(){
    while (!Serial){ data_sent=false; delay(10); } // Checking if anyone is still connected.
}

void Teensy::send_receive(){
